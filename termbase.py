...
class termCRM:
    ...
    def print_menu(self, selected_idx):
        ...
        menu_items = ['View Contacts', 'Add Contact', 'Delete Contact', 'Search Contacts', 'Export Contacts', 'Exit']
    ...
    def add_contact(self):
        ...
        self.contacts_db.execute("INSERT INTO contacts (NAME, PHONE, NOTE, EMAIL) VALUES (?, ?, ?, ?)", (name, phone, note, email))
    ...
    def search_contacts(self):
        ...
        contacts = self.contacts_db.execute(f"SELECT * FROM contacts WHERE NAME LIKE '%{query}%' OR NOTE LIKE '%{query}%' OR EMAIL LIKE '%{query}%'")
        for i, contact in enumerate(contacts):
            contact_str = f"{contact[0]}. {contact[1]} ({contact[2]}, {contact[4]}) Notes: {contact[3]}"
